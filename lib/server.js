...
const HEALTHCHECK_URL = process.env.PRERENDER_HEALTHCHECK_URL || 'https://www.google.com/generate_204';
const HEALTHCHECK_TIMEOUT = parseInt(process.env.PRERENDER_HEALTHCHECK_TIMEOUT, 10) || 5000;

const envConsecutiveErrorThreshold = parseInt(
  process.env.PRERENDER_CONSECUTIVE_ERROR_THRESHOLD,
  10,
);
const CONSECUTIVE_RENDER_ERROR_THRESHOLD =
  Number.isNaN(envConsecutiveErrorThreshold) || envConsecutiveErrorThreshold < 0
    ? 5
    : envConsecutiveErrorThreshold;
const envHealthConsecutiveErrorThreshold = parseInt(
  process.env.PRERENDER_HEALTHCHECK_CONSECUTIVE_ERROR_THRESHOLD,
  10,
);
const HEALTHCHECK_CONSECUTIVE_ERROR_THRESHOLD =
  Number.isNaN(envHealthConsecutiveErrorThreshold) ||
  envHealthConsecutiveErrorThreshold < 0
    ? CONSECUTIVE_RENDER_ERROR_THRESHOLD
    : envHealthConsecutiveErrorThreshold;
const envHealthMaxStaleMs = parseInt(
  process.env.PRERENDER_HEALTHCHECK_MAX_STALE_MS,
  10,
);
const HEALTHCHECK_MAX_STALE_MS =
  Number.isNaN(envHealthMaxStaleMs) || envHealthMaxStaleMs <= 0
    ? 0
    : envHealthMaxStaleMs;
const server = {};
...
server.init = function (options) {
  ...
  this.options.healthCheckUrl =
    this.options.healthCheckUrl || HEALTHCHECK_URL;
  this.options.healthCheckTimeout =
    this.options.healthCheckTimeout || HEALTHCHECK_TIMEOUT;

  const consecutiveOption = parseInt(
    this.options.consecutiveErrorThreshold,
    10,
  );
  this.options.consecutiveErrorThreshold =
    Number.isNaN(consecutiveOption) || consecutiveOption < 0
      ? CONSECUTIVE_RENDER_ERROR_THRESHOLD
      : consecutiveOption;

  const healthConsecutiveOption = parseInt(
    this.options.healthCheckConsecutiveErrorThreshold,
    10,
  );
  this.options.healthCheckConsecutiveErrorThreshold =
    Number.isNaN(healthConsecutiveOption) || healthConsecutiveOption < 0
      ? HEALTHCHECK_CONSECUTIVE_ERROR_THRESHOLD
      : healthConsecutiveOption;

  const healthMaxStaleOption = parseInt(
    this.options.healthCheckMaxStaleMs,
    10,
  );
  this.options.healthCheckMaxStaleMs =
    Number.isNaN(healthMaxStaleOption) || healthMaxStaleOption < 0
      ? HEALTHCHECK_MAX_STALE_MS
      : healthMaxStaleOption;
  ...
  return new Promise((resolve, reject) => {
    this.spawnBrowser()
      ...
      .then(() => {
        ...
        if (this.healthState) {
          this.healthState.restartInProgress = false;
        }
...
server.restartBrowser = function (reason) {
  if (this.restartRequested) {
    util.log(
      'Browser restart already pending',
      'reason=' + (this.pendingRestartReason || reason || 'unknown'),
    );
    return;
  }
  ...
  this.isBrowserConnected = false;
  if (this.healthState) {
    this.healthState.restartInProgress = true;
    this.healthState.lastRestartReason = reason || 'manual';
    this.healthState.lastRestartAt = Date.now();
  }
  this.restartRequested = true;
  this.pendingRestartReason = reason || 'manual';
  this.spawningBrowser = false;
  const pid = this.browser.getPid ? this.browser.getPid() : undefined;
  const inFlight = this.browserRequestsInFlight
    ? this.browserRequestsInFlight.size
  ...
  if (!pid) {
    util.log('Browser PID missing during restart, starting a fresh instance');
    this.restartRequested = false;
    this.pendingRestartReason = null;
    this.startPrerender().catch((err) => {
      util.log('Failed to restart browser without active PID', err);
      if (process.exit) {
        process.exit();
      }
    });
...
server.healthCheck = async function (options = {}) {
  const healthCheckId = uuidv4();
  const targetUrl = this.options.healthCheckUrl;
  const inFlight = this.browserRequestsInFlight
    ? this.browserRequestsInFlight.size
    : 0;
  const now = Date.now();
  const healthState = this.healthState || {};
  ...
  const status = {
    ...
    targetUrl,
    consecutiveRenderErrors: healthState.consecutiveRenderErrors || 0,
    lastErrorAt: healthState.lastErrorAt || null,
    lastErrorReason: healthState.lastErrorReason || null,
    lastErrorStatusCode: healthState.lastErrorStatusCode || null,
    lastErrorUrl: healthState.lastErrorUrl || null,
    lastErrorStreak: healthState.lastErrorStreak || 0,
    lastSuccessfulRenderAt: healthState.lastSuccessfulRenderAt || null,
    lastRestartAt: healthState.lastRestartAt || null,
    lastRestartReason: healthState.lastRestartReason || null,
    restartInProgress: !!healthState.restartInProgress,
    checkedAt: now,
  };

  if (status.lastSuccessfulRenderAt) {
    status.lastSuccessfulRenderAgoMs = now - status.lastSuccessfulRenderAt;
  }
  ...
  if (!this.isBrowserConnected) {
    const error = new Error('Browser is not connected');
    error.code = 'HEALTHCHECK_BROWSER_DISCONNECTED';
    error.details = status;
    util.log('healthcheck failure', 'checkId=' + healthCheckId, error);
    throw error;
  }
  ...
  const errorThreshold = this.options.healthCheckConsecutiveErrorThreshold;
  if (
    typeof errorThreshold === 'number' &&
    errorThreshold > 0 &&
    status.consecutiveRenderErrors >= errorThreshold
  ) {
    const error = new Error(
      'Consecutive render errors ' +
        status.consecutiveRenderErrors +
        ' exceed threshold ' +
        errorThreshold,
    );
    error.code = 'HEALTHCHECK_CONSECUTIVE_ERRORS';
    error.details = status;
    util.log('healthcheck failure', 'checkId=' + healthCheckId, error);
    throw error;
  }

  const maxStaleMs = this.options.healthCheckMaxStaleMs;
  if (
    typeof maxStaleMs === 'number' &&
    maxStaleMs > 0 &&
    status.lastSuccessfulRenderAt &&
    status.lastSuccessfulRenderAgoMs > maxStaleMs
  ) {
    const error = new Error(
      'Last successful render ' +
        status.lastSuccessfulRenderAgoMs +
        'ms ago exceeds threshold ' +
        maxStaleMs +
        'ms',
    );
    error.code = 'HEALTHCHECK_STALE_RENDER';
    error.details = status;
    util.log('healthcheck failure', 'checkId=' + healthCheckId, error);
    throw error;
  }
  ...
  if (!targetUrl) {
    util.log('healthcheck ok', 'checkId=' + healthCheckId, 'skippedExternal=true');
    status.latencyMs = 0;
    return status;
  }
  ...
  try {
    const { statusCode } = await probeUrl(
      targetUrl,
      this.options.healthCheckTimeout,
    );
    const latency = Date.now() - start;
    status.externalStatusCode = statusCode;
    ...
    if (typeof err.statusCode === 'number') {
      status.externalStatusCode = err.statusCode;
    }
    if (err.code) {
      status.externalErrorCode = err.code;
    }
    status.externalError = err.message;
    const failure = new Error('Healthcheck failed: ' + err.message);
    failure.code = err.code || 'HEALTHCHECK_EXTERNAL_FAILURE';
    failure.details = status;
    failure.cause = err;
    util.log('healthcheck failure', 'checkId=' + healthCheckId, err);
    throw failure;
  }
};
...
server.listenForBrowserClose = function () {
  ...
  this.browser.onClose((code, signal, pid) => {
    this.isBrowserConnected = false;
    const restartRequested = this.restartRequested;
    const restartReason = this.pendingRestartReason;
    this.restartRequested = false;
    this.pendingRestartReason = null;
    ...
    if (uptime < 1000 && !restartRequested) {
      util.log(
    ...
    if (restartRequested) {
      util.log(
        `${this.browser.name} restart requested... starting a fresh instance`,
    ...
    this.browserStartedAt = null;
    this.startPrerender().catch((err) => {
      util.log('Failed to restart Chrome after close', err);
      if (process.exit) {
        process.exit();
      }
    });
...
server.recordRequestOutcome = function (req) {
  if (!this.healthState) {
    return;
  }

  const statusCode =
    parseInt(req.prerender.statusCode, 10) || this.options.renderErrorStatusCode;
  const timedOut = !!req.prerender.timedout;
  // Reaching PAGE_LOAD_TIMEOUT is a normal successful fallback: prerender
  // returns the HTML captured at that point. Only the outer request timeout
  // marks the request cancelled and should count as a render failure.
  const requestTimedOut = timedOut && !!req.prerender.cancelled;
  const chromeErrors = Array.isArray(req.prerender.errors)
    ? req.prerender.errors.filter(Boolean)
    : [];
  const hadChromeErrors = chromeErrors.length > 0;
  const isRenderErrorStatus = statusCode === this.options.renderErrorStatusCode;
  const failed = requestTimedOut || hadChromeErrors || isRenderErrorStatus;

  const now = Date.now();

  if (failed) {
    this.healthState.consecutiveRenderErrors += 1;
    this.healthState.lastErrorAt = now;
    this.healthState.lastErrorStatusCode = statusCode;
    this.healthState.lastErrorReason =
      req.prerender.statusCodeReason ||
      (requestTimedOut
        ? 'request timed out'
        : hadChromeErrors
        ? chromeErrors.join(',')
        : 'status=' + statusCode);
    this.healthState.lastErrorUrl = req.prerender.url;
    this.healthState.lastErrorStreak = this.healthState.consecutiveRenderErrors;

    util.log(
      'render error tracked',
      'consecutive=' + this.healthState.consecutiveRenderErrors,
      'reqId=' + req.prerender.reqId,
      'status=' + statusCode,
      'reason=' + this.healthState.lastErrorReason,
      'url=' + req.prerender.url,
    );

    const threshold = this.options.consecutiveErrorThreshold;
    if (
      typeof threshold === 'number' &&
      threshold > 0 &&
      this.healthState.consecutiveRenderErrors >= threshold &&
      !this.healthState.restartInProgress
    ) {
      util.log(
        'render error threshold exceeded',
        'threshold=' + threshold,
        'action=restart',
        'reqId=' + req.prerender.reqId,
        'url=' + req.prerender.url,
      );
      this.restartBrowser('consecutive-render-errors');
    }
  } else {
    if (this.healthState.consecutiveRenderErrors > 0) {
      util.log(
        'render error streak reset',
        'previous=' + this.healthState.consecutiveRenderErrors,
      );
    }
    this.healthState.consecutiveRenderErrors = 0;
    this.healthState.lastErrorStreak = 0;
    this.healthState.lastSuccessfulRenderAt = now;
  }
};

function probeUrl(targetUrl, timeoutMs) {
  let parsed;
  try {
    parsed = new URL(targetUrl);
  } catch (err) {
    err.code = err.code || 'HEALTHCHECK_INVALID_URL';
    throw err;
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      headers: {
        'user-agent': 'prerender-healthcheck',
      },
    };
    ...
    const req = requestModule.request(options, (res) => {
      ...
      if (res.statusCode >= 200 && res.statusCode < 400) {
        ...
        const error = new Error(
          `Unexpected status code ${res.statusCode} for healthcheck`,
        );
        error.code = 'HEALTHCHECK_BAD_STATUS';
    ...
    req.setTimeout(timeoutMs, () => {
      const timeoutError = new Error(
        `Healthcheck timed out after ${timeoutMs}ms`,
      );
