...
exports = module.exports = (
  ...
  app.get(healthCheckPath, async (req, res) => {
    ...
    try {
      // Kubernetes/Northflank probes should only test whether this process can
      // serve renders. Page-level failures and the optional external probe are
      // diagnostics; treating them as liveness failures causes healthy pods to
      // be restarted during ordinary page-load timeouts.
      const checks = await server.healthCheck({ liveness: true });
