...
const FORCE_KILL_TIMEOUT_MS =
  parseInt(process.env.CHROME_FORCE_KILL_TIMEOUT_MS, 10) || 10000;

chrome.getChromeFlags = function (options) {
  const tempDirectory = path.resolve(os.tmpdir());
  const userDataDir = path.resolve(
    options.chromeUserDataDir || path.join(tempDirectory, 'prerender-chrome'),
  );

  if (
    userDataDir === tempDirectory ||
    !userDataDir.startsWith(tempDirectory + path.sep)
  ) {
    throw new Error(
      'Chrome user data directory must be inside the temp directory',
    );
  }

  // Chromium creates a new scoped profile in the user's home directory on
  // every forced restart. Those abandoned profiles eventually fill the pod's
  // ephemeral storage, so keep one managed profile and reset it before launch.
  ...
  let chromeFlags = [
    '--headless',
    '--disable-gpu',
    '--remote-debugging-port=' + options.browserDebuggingPort,
    '--hide-scrollbars',
    '--user-data-dir=' + userDataDir,
  ];

  if (options.extraChromeFlags) {
    chromeFlags = chromeFlags.concat(options.extraChromeFlags);
  }
  if (options.proxyServer) {
    chromeFlags.push('--proxy-server=' + options.proxyServer);
  }
...
chrome.spawn = function (options) {
  return new Promise((resolve, reject) => {
    ...
    const chromeFlags =
      this.options.chromeFlags || this.getChromeFlags(this.options);

    util.log(
      'Launching Chrome process',
      'executable=' + location,
      'flags=' + chromeFlags.join(' '),
    );

    this.chromeKillTimer = null;
    ...
    const childPid = this.chromeChild.pid;
    this.chromeChild.once('spawn', () => {
      util.log('Chrome child spawned', 'pid=' + this.chromeChild.pid);
    });
    this.chromeChild.on('error', (err) => {
      util.log('Chrome child process error', 'pid=' + childPid, err);
    });
...
chrome.onClose = function (callback) {
  if (!this.chromeChild) {
    util.log('Chrome onClose requested but chromeChild is not set');
    return;
  }

  this.chromeChild.once('close', (code, signal) => {
    if (this.chromeKillTimer) {
      clearTimeout(this.chromeKillTimer);
      this.chromeKillTimer = null;
    }

    const child = this.chromeChild;
    const pid = child && child.pid;
    const formattedCode = code === null || code === undefined ? 'null' : code;
    util.log(
      'Chrome child close event',
      'pid=' + (pid || '-'),
      'code=' + formattedCode,
      'signal=' + (signal || '-'),
    );
...
chrome.kill = function () {
  if (!this.chromeChild) {
    util.log('No Chrome child process to kill');
    return;
  }

  const pid = this.chromeChild.pid;
  let signal = 'SIGINT';
  util.log(`Sending ${signal} to browser PID group ${pid}`);
  try {
    process.kill(-pid, signal); // negative PID means the whole group
  } catch (err) {
    util.log('error sending SIGINT to Chrome', err);
  }
  ...
  if (FORCE_KILL_TIMEOUT_MS > 0) {
    this.chromeKillTimer = setTimeout(() => {
      if (!this.chromeChild || this.chromeChild.pid !== pid) {
        return;
      }

      util.log(
        `Chrome PID ${pid} still running after ${FORCE_KILL_TIMEOUT_MS}ms, sending SIGKILL`,
      );
      try {
        process.kill(-pid, 'SIGKILL');
      } catch (err) {
        util.log('error sending SIGKILL to Chrome', err);
      }
    }, FORCE_KILL_TIMEOUT_MS);

    if (this.chromeKillTimer && this.chromeKillTimer.unref) {
      this.chromeKillTimer.unref();
    }
  }
};

chrome.getPid = function () {
  return this.chromeChild ? this.chromeChild.pid : undefined;
};
...
chrome.loadUrlThenWaitForPageLoadEvent = function (tab, url, onNavigated) {
  return new Promise((resolve, reject) => {
    ...
    const abortIfCancelled = () => {
      ...
      if (!tab.prerender.cancelled) {
        return false;
      }

      finished = true;

      util.log(`Page load cancelled url=${tab.prerender.url}`);

      Page.stopLoading().catch((err) => {
        util.log(
          `unable to stop loading page (cancelled), url=${tab.prerender.url}`,
          err,
        );
      });

      const cancelError = new Error('Request cancelled');
      cancelError.code = 'PRERENDER_REQUEST_TIMEOUT';
      reject(cancelError);
      return true;
    };
